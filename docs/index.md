# Redes Neurais e Deep Learning

???+ info inline end "Informações do Curso"
    **Instituição:** Insper  
    **Semestre:** 2025.2  
    **Professor:** Humberto Sandmann  
    **Aluno:** Lucas Fernando de Souza Lima  

Este site reúne os materiais, anotações e entregas da disciplina **Redes Neurais e Deep Learning**, abordando desde conceitos fundamentais (como perceptrons e MLPs) até modelos mais avançados, como autoencoders variacionais (VAE).


## Exercícios

- [x] [Data](./exercicios/data/main.md)
- [x] [Perceptron](./exercicios/perceptron/main.md)
- [x] [MLP](./exercicios/mlp/index.ipynb)
- [ ] VAE<!--[VAE] (vae/main.md)-->

## Projetos

- [x] [Classificação](https://lucasouzamil.github.io/mlp-classification/#72-analysis-of-learning-behavior)
- [x] [Regressão](https://lucasouzamil.github.io/ann-regression/)
- [ ] Generative<!--[VAE] (vae/main.md)-->